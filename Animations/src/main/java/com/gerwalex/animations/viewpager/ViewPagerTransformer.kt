...
internal class ViewPagerTransformer(
    private val transformType: ViewPagerTransformerType,
    ...
    override fun transformPage(page: View, position: Float) {
        when (transformType) {
            ViewPagerTransformerType.Flow -> {
                page.rotationY = position * FLOW_ROTATION_ANGLE
            }
            ViewPagerTransformerType.SlideOver -> transformSlideOver(position, page)
            ViewPagerTransformerType.Depth -> transformDepth(position, page)
            ViewPagerTransformerType.Zoom -> transformZoom(position, page)
            ViewPagerTransformerType.Fade -> transformFade(position, page)
            is ViewPagerTransformerType.Parallax -> {
